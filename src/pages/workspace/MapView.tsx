...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    const checkLeaflet = () => {
      if ((window as any).L) { setScriptsLoaded(true); clearInterval(checkInterval); }
    };
    if (!document.getElementById('leaflet-script')) {
      const script = document.createElement('script');
      script.id = 'leaflet-script'; script.src = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'; script.async = true;
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      ...
      L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '© OSM' }).addTo(mapInstanceRef.current);
    }
    ...
    const scheduledIdeas = [...ideas].filter(i => i.start_datetime).sort((a, b) => new Date(a.start_datetime).getTime() - new Date(b.start_datetime).getTime());
    scheduledIdeas.forEach(idea => {
      if (idea.location_lat && idea.location_lng) routePoints.push([idea.location_lat, idea.location_lng]);
      if (idea.category === 'Transportation' && idea.end_location_lat && idea.end_location_lng) routePoints.push([idea.end_location_lat, idea.end_location_lng]);
    });

    if (routePoints.length > 1) L.polyline(routePoints, { color: '#ef4444', weight: 4, dashArray: '10, 10', opacity: 0.8 }).addTo(map);

    ideas.forEach(idea => {
      const createPopup = (label: string) => `<div style="min-width: 150px; padding: 4px;"><h4 style="font-weight: bold; margin: 0 0 4px 0; font-size: 14px;">${idea.title} ${label}</h4><span style="font-size: 10px; background: #f1f5f9; padding: 2px 6px; border-radius: 4px; color: black;">${idea.category}</span><br/><button onclick="window.dispatchEvent(new CustomEvent('openCard', {detail: '${idea.id}'}))" style="margin-top: 8px; width: 100%; background: #0f172a; color: white; border: none; padding: 6px; border-radius: 4px; cursor: pointer;">View Details</button></div>`;
      if (idea.location_lat && idea.location_lng) {
        localHasPoints = true; bounds.extend([idea.location_lat, idea.location_lng]);
        L.marker([idea.location_lat, idea.location_lng]).addTo(map).bindPopup(createPopup(idea.category === 'Transportation' ? '(Origin)' : ''));
      }
      if (idea.category === 'Transportation' && idea.end_location_lat && idea.end_location_lng) {
        localHasPoints = true; bounds.extend([idea.end_location_lat, idea.end_location_lng]);
        L.marker([idea.end_location_lat, idea.end_location_lng]).addTo(map).bindPopup(createPopup('(Destination)'));
      }
    });

    setHasPlottedPoints(localHasPoints);
    if (localHasPoints && activeView === 'custom') setTimeout(() => map.flyToBounds(bounds, { padding: [50, 50], duration: 1.5 }), 500);
  ...
  const carouselIdeas = [...ideas].filter(i => i.start_datetime && i.location_address).sort((a, b) => new Date(a.start_datetime).getTime() - new Date(b.start_datetime).getTime());
      ...
      <div className="h-[calc(100vh-73px)] flex flex-col relative w-full bg-muted/20 overflow-hidden">
        <div className="absolute top-6 left-6 right-6 z-[400] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pointer-events-none">
          <div className="bg-card/90 backdrop-blur border p-4 rounded-xl shadow-lg max-w-sm pointer-events-auto">
            ...
            <p className="text-sm text-muted-foreground mt-1 leading-tight">{activeView === 'custom' ? 'A macro-level view of all geocoded coordinates.' : 'A chronological slideshow of your itinerary with live Google Maps routing.'}</p>
          </div>
          <div className="pointer-events-auto shadow-lg rounded-lg bg-card/90 backdrop-blur">
            <Tabs value={activeView} onValueChange={setActiveView} className="w-full md:w-[350px]">
              <TabsList className="grid w-full grid-cols-2 p-1"><TabsTrigger value="custom">Custom Global Map</TabsTrigger><TabsTrigger value="carousel">Google Carousel</TabsTrigger></TabsList>
            </Tabs>
          </div>
        </div>
          ...
          <div className="absolute inset-0 z-10 pt-[160px] pb-8 flex items-center overflow-x-auto snap-x snap-mandatory px-8 gap-8 custom-scrollbar">
             {carouselIdeas.length === 0 ? (
                <div className="w-full flex items-center justify-center text-muted-foreground flex-col h-full">
                   ...
                   <p className="mt-2 text-center max-w-md">Your Idea Cards must have a Location Address AND be scheduled on the Timeline.</p>
                </div>
             ) : (
                carouselIdeas.map((idea, idx) => (
                   <div key={idea.id} className="min-w-[85vw] md:min-w-[60vw] lg:min-w-[50vw] h-full max-h-[75vh] shrink-0 snap-center bg-card rounded-2xl border shadow-2xl flex flex-col overflow-hidden relative">
                       <div className="absolute top-4 left-4 z-20 bg-primary text-primary-foreground h-10 w-10 rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-background">{idx + 1}</div>
                           ...
                           <div className="flex justify-between items-start gap-4">
                             <div>
                               ...
                               <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-muted-foreground">
                                  ...
                                  {idea.start_datetime && <span className="flex items-center gap-1 font-medium bg-background px-2 py-0.5 rounded-md border shadow-sm"><Clock className="h-3.5 w-3.5 text-primary"/> {format(parseISO(idea.start_datetime), 'MMM d • h:mm a')}</span>}
                               </div>
                             </div>
                       ...
                       <div className="flex-1 w-full bg-muted relative">
                          <iframe width="100%" height="100%" style={{ border: 0 }} loading="lazy" allowFullScreen src={idea.category === 'Transportation' && idea.end_location_address ? `https://maps.google.com/maps?saddr=${encodeURIComponent(idea.location_address)}&daddr=${encodeURIComponent(idea.end_location_address)}&output=embed` : `https://maps.google.com/maps?q=${encodeURIComponent(idea.location_address)}&output=embed`} />
                       </div>
                   </div>
                ))
             )}
