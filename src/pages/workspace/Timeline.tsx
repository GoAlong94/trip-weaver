...
const VERSIONS = [{ id: 'idea', label: 'Brainstorming' }, { id: 'draft_a', label: 'Draft A' }, { id: 'draft_b', label: 'Draft B' }, { id: 'active', label: 'Active Itinerary' }];
...
export default function Timeline() {
  ...
  const { ideas: globalIdeas, members, loading: globalLoading, refreshData } = useTripData(tripId);
  const membersCount = members.length || 1;

  // Local synced ideas for instantaneous Drag & Drop
  const [ideas, setIdeas] = useState<any[]>([]);
  useEffect(() => { setIdeas(globalIdeas); }, [globalIdeas]);
  ...
  useEffect(() => {
    if (trip && !globalLoading && !interaction) {
  ...
  }, [trip, ideas, globalLoading, activeTab, interaction]);
  ...
  const handleAddIdea = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newIdea = { trip_id: tripId, title, category, unit_cost: unitCost, created_by: user.id, draft_version: activeTab, visibility, shared_with: finalSharedWith, quantity: 1, quantity_type: 'fixed', upvotes: [] };
      const { error } = await supabase.from('idea_cards').insert([newIdea]);
      if (error) throw error;
      toast.success('Idea added to pool!');
      setShowForm(false); setTitle(''); setUnitCost(0); setVisibility('public'); setSharedWith([]);
      refreshData();
  ...
  const handlePointerDown = (e: React.MouseEvent, type: 'move' | 'resize-left' | 'resize-right', idea: any) => {
    e.stopPropagation(); isDraggingRef.current = false;
  ...
  const updateIdeaDates = async (id: string, start: Date, end: Date) => {
    ...
    refreshData(); // Sync DB updates seamlessly in background
  };

  if (globalLoading || !baseStart) return <div className="p-8 flex justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  ...
  const packLane = (items: any[]) => {
    ...
    return sorted.map(item => {
      const startPx = differenceInMinutes(parseISO(item.start_datetime), baseStart) * pixelsPerMinute;
      const actualWidthPx = Math.max(MIN_CARD_WIDTH, differenceInMinutes(parseISO(item.end_datetime), parseISO(item.start_datetime)) * pixelsPerMinute);
      const endPx = startPx + actualWidthPx;
      let rowIndex = 0;
      while (rows[rowIndex] !== undefined && rows[rowIndex] > startPx - 10) rowIndex++;
      rows[rowIndex] = endPx;
  ...
  return (
    <div className="p-6 h-[calc(100vh-73px)] flex flex-col overflow-hidden relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 shrink-0">
        ...
        <Button onClick={() => setShowForm(!showForm)} className="gap-2">{showForm ? 'Cancel' : <><Plus className="h-4 w-4"/> Propose Idea</>}</Button>
      </div>
        ...
        <Card className="mb-6 border-primary/20 bg-muted/30 shadow-inner shrink-0">
          <CardContent className="pt-6">
            <form onSubmit={handleAddIdea} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
              <div className="space-y-2 lg:col-span-2"><Label>Idea Title</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="e.g. Secret Beer Run" /></div>
              ...
              <div className="space-y-2"><Label>Visibility</Label><Select value={visibility} onValueChange={setVisibility}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="public"><div className="flex items-center gap-2"><Eye className="h-3 w-3"/> Public</div></SelectItem><SelectItem value="subgroup"><div className="flex items-center gap-2"><Users className="h-3 w-3"/> Sub-Group</div></SelectItem><SelectItem value="private"><div className="flex items-center gap-2"><EyeOff className="h-3 w-3"/> Private</div></SelectItem></SelectContent></Select></div>
              <div className="space-y-2"><Label>Est. Cost (₹)</Label><Input type="number" min="0" value={unitCost} onChange={(e) => setUnitCost(Number(e.target.value))} required /></div>
      ...
      <div className="flex gap-4 flex-1 min-h-0 overflow-hidden">
        {unscheduledIdeas.length > 0 && (
          <div className="w-64 flex flex-col bg-muted/30 rounded-xl border shrink-0 overflow-hidden shadow-inner">
            <div className="p-3 border-b bg-muted/50 font-semibold text-sm uppercase flex items-center justify-between"><span className="flex items-center gap-2"><GripVertical className="h-4 w-4" /> Unscheduled</span><Badge variant="secondary">{unscheduledIdeas.length}</Badge></div>
            <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
              {unscheduledIdeas.map(idea => (
                <Card key={idea.id} draggable onDragStart={(e) => handleDragStartPool(e, idea.id)} className="cursor-grab active:cursor-grabbing hover:border-primary/50 shadow-sm border-l-4" style={{ borderLeftColor: 'var(--primary)' }}>
                  <CardContent className="p-3">
                    <h4 className="font-medium text-sm leading-tight">{idea.title}</h4>
                    <div className="flex items-center gap-2 mt-2"><Badge variant="secondary" className="text-[10px] px-1.5 py-0">{idea.category}</Badge></div>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <div ref={timelineRef} className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar bg-background rounded-xl border relative select-none shadow-inner" onDragOver={(e) => e.preventDefault()} onDrop={handleDropOnTimeline}>
          <div style={{ width: timelineHours.length * pixelsPerHour, minHeight: '100%' }} className="relative flex flex-col">
            ...
            <div className="absolute inset-0 top-[64px] flex pointer-events-none z-0">
              {timelineHours.map((_, i) => <div key={i} style={{ width: pixelsPerHour }} className="shrink-0 border-r border-border/20 h-full bg-muted/5" />)}
            </div>
            <div className="relative z-10 flex-1 flex flex-col pt-6 pb-12">
              {CATEGORIES.map(category => {
                ...
                return (
                  <div key={category} style={{ minHeight: Math.max(90, (maxRow + 1) * 64 + 40) }} className="relative w-full group mb-4">
                    <div className="sticky left-4 inline-block px-3 py-1 bg-background/90 backdrop-blur-sm rounded-md border text-xs font-bold text-muted-foreground uppercase z-30 shadow-sm mb-2">{category}</div>
                    {laneItems.map(idea => (
                        <div key={idea.id} style={{ left: idea.startPx, width: idea.actualWidthPx, top: 40 + (idea.rowIndex * 60), position: 'absolute' }} className={`h-12 rounded-xl border backdrop-blur-sm shadow-sm transition-all group/card flex ${CATEGORY_COLORS[idea.category] || CATEGORY_COLORS['Other']} ${interaction?.id === idea.id ? 'z-50 ring-2 ring-primary scale-[1.02] shadow-xl' : 'z-20'}`}>
                          <div className="w-3 shrink-0 cursor-ew-resize flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/10 rounded-l-xl z-20" onMouseDown={(e) => handlePointerDown(e, 'resize-left', idea)}><div className="w-1 h-4 bg-foreground/20 rounded-full" /></div>
                          <div className="flex-1 relative h-full flex items-center cursor-grab active:cursor-grabbing z-10" style={{ overflow: 'clip' }} onMouseDown={(e) => handlePointerDown(e, 'move', idea)} onClick={() => { if (!isDraggingRef.current) setSelectedIdea(idea); }}>
                              <div className="sticky left-2 z-30 pointer-events-none flex flex-col justify-center drop-shadow-sm px-2 w-max max-w-full"><div className="text-sm font-bold truncate tracking-tight">{idea.title}</div><div className="text-[10px] font-semibold opacity-80 truncate">{format(parseISO(idea.start_datetime), 'h:mm a')} - {format(parseISO(idea.end_datetime), 'h:mm a')}</div></div>
                          </div>
                          <button onClick={(e) => removeFromTimeline(idea.id, e)} className="absolute -top-2 -right-2 bg-destructive text-destructive-foreground rounded-full p-1 opacity-0 group-hover/card:opacity-100 transition-opacity hover:scale-110 z-40 shadow-xl"><X className="h-3 w-3" /></button>
                          <div className="w-3 shrink-0 cursor-ew-resize flex items-center justify-center hover:bg-black/10 dark:hover:bg-white/10 rounded-r-xl z-20" onMouseDown={(e) => handlePointerDown(e, 'resize-right', idea)}><div className="w-1 h-4 bg-foreground/20 rounded-full" /></div>
                        </div>
                      ))}
                  </div>
                );
      ...
      <IdeaCardModal idea={selectedIdea} isOpen={!!selectedIdea} onClose={() => setSelectedIdea(null)} onUpdate={refreshData} memberCount={membersCount} />
    </div>
  );
}
